...
const AdminClassroomView = () => {
  ...
  useEffect(() => {
    if (classroom && location.pathname === `/admin/classrooms/${id}`) {
      navigate(`/admin/classrooms/${id}/stream`, { replace: true });
    }
  ...
  const fetchClassroomDetails = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/admin/classrooms/${id}`,
        {
          headers: {
  ...
  if (isLoading || !classroom)
    return <GlobalSpinner isLoading={true} text="Loading Classroom..." />;
...
export default AdminClassroomView;
